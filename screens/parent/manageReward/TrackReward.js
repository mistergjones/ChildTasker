...
function TrackReward({ navigation }) {
  ...
  const getRewardsHaveBeenAssignedToKid = (kidName, rewardsForKid) => {
    ...
    rewards.map(reward => console.log("reward name = " + reward.reward_name))
    rewardsForKid.map(rfk => console.log("reward name = " + rfk.reward_name + "rfk.uniqueID = " + rfk.rewardUniqueId))
    // loop through chores
    for (let i = 0; i < rewardsForKid.length; i++) {
      ...
      for (let x = 0; x < rewards.length; x++) {
        ...
        let reward = rewards[x];
        let returnReward = {};
        console.log("rewardsForKid[i].rewardID = " + rewardsForKid[i].rewardID)
        ...
        if (rewardsForKid[i].rewardID === reward.reward_id) {
          ...
          console.log("rewardsForKid[i].rewardUniqueId" + rewardsForKid[i].rewardUniqueId)
          // returnReward = reward;
