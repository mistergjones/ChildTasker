...
function ChoreProgressScreen({ navigation, route }) {
    ...
    let { chores, chore_id } = route.params;
    ...
    var [tasksNotComplete, setTasksNotComplete] = useState(0);
    ...
    useEffect(() => {
        ...
        chores.map((chore, index) => {
            if (chore.is_completed === 1 || chore.chore_id === chore_id) {
                completed += 1;
                chores[index].is_completed = 1;
            } else {
                notCompleted += 1;
            }
        });
        setGraphData(changeChoresToPieChartDataObject(chores));
        setTasksComplete(completed);
        setTasksNotComplete(notCompleted);
    }, []);
