...
export const establishTasksListInObjectFormat = (itemObject) => {
    ...
    for (
        ...
        var tempObject = {};
        tempObject.label = itemObject[loopIterator].task_name;
        tempObject.value = itemObject[loopIterator].task_id;
        tempObject.backgroundColor = itemObject[loopIterator].task_colour;
        tempObject.icon = itemObject[loopIterator].task_icon;
        tempObject.category_id = itemObject[loopIterator].category_id;
        finalisedList.push(tempObject);
    }
    ...
    for (var loopIterator = 0; loopIterator < chores.length; loopIterator++) {
        ...
        tempObject.svg =
            chores[loopIterator].is_completed === 1
                ? { fill: Color.pieChartCompletedColor }
                : { fill: Color.pieChartInCompletedColor };
